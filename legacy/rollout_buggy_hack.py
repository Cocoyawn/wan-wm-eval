...
parser.add_argument("--out_root", type=str, default="outputs/eval_tower_of_hanoi_epoch99")
...
def generate_sequence(rgb_list, actions, condition_frames=CONDITION_FRAMES,
    ...
    for i in range(num_iters):
        ...
        # reference 帧动作约定（与原脚本一致）：首帧动作置 0，末维置 -1
        actions[0] = 0
        actions[0, -1] = -1
...
def process_one_sequence(rel_path, out_root, save_png=False, save_gt=False):
    ...
    t0 = time.time()
    gen_frames = generate_sequence(rgb_list, actions)
